...
export function PinScreen({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <div className="flex h-full flex-col justify-center gap-10 bg-page px-7">
      <p className="stack-in font-serif text-h1 text-ink">Ontgrendel Father Flow</p>

      <div
        className={`stack-in stack-delay-1 flex justify-center gap-4 ${shake ? 'animate-[shake_0.4s_ease-in-out]' : ''}`}
      >
      ...
      <div className="stack-in stack-delay-2 grid grid-cols-3 gap-5">
        {KEYS.map((key) => {
          if (key === 'face') {
            return (
              <button
