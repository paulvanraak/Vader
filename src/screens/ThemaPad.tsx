...
import type { Thema } from '../content/themas'
 ...
 */
export function ThemaPad({ thema, ritme, onOpen, onKlaar }: {
  thema: Thema
  ritme: RitmeState
  ...
  // Tellen binnen dít thema: voltooid bevat ook delen uit eerdere thema's.
  const gedaan = ids.filter((id) => ritme.voltooid.includes(id)).length
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="px-5 pb-5">
        <h1 className="font-serif text-h2 font-semibold text-ink">{p(thema.titel)}</h1>
        <p className="mt-1 text-body text-ink-muted">{p(thema.ondertitel)}</p>
        <p className="mt-2 text-caption text-ink-muted">
          {gedaan} van {lessen.length} delen afgerond
        </p>
      </div>

      <div className="flex flex-col px-5 pb-8">
        {lessen.map((les, i) => {
          const klaar = ritme.voltooid.includes(les.id)
          const nu = !klaar && i === open - 1
          const dicht = !klaar && !nu
          const aantal = thema.oefeningen.filter((o) => o.lesId === les.id).length
          ...
          return (
            <div key={les.id} className="flex gap-3">
              <div className="flex w-10 shrink-0 flex-col items-center">
                ...
                >
                  {klaar ? <Check size={18} strokeWidth={3} />
                    : nu ? <Play size={16} strokeWidth={2.5} className="ml-0.5" />
                    : <Lock size={15} strokeWidth={2} />}
                </button>
                {i < lessen.length - 1 && (
                  <div className={`h-14 border-l-2 border-dashed ${klaar ? 'border-ink' : 'border-ink-faint'}`} />
                )}
              </div>
