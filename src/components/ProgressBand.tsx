...
export function ProgressBand({ completed, total }: ProgressBandProps) {
  ...
  return (
    <div
      ...
      className="rounded-2xl bg-surface p-5 shadow-sm ring-1 ring-surface-sunken"
    >
      <div className="flex items-center gap-2">
        <HeartHandshake size={18} className="text-accent-orange" strokeWidth={2} />
        <span className="text-label text-ink">Jullie band</span>
        <span className="ml-auto text-caption text-ink-muted">{stages[stageIndex]}</span>
      </div>
